...
R5.7.41.3.4.10.10.1 formalizes a deployment boundary:

* PhysicsCore is the authoring/calibration/validation authority.
* WINDY is a standalone consumer and MUST NOT require a running PhysicsCore
  process, Python interpreter, Streamlit app, or PhysicsCore API.
* The portable package contains only a calibrated six-band LUT, a frozen
  contract, checksums, cross-language validation vectors, and dependency-free
  TypeScript/JavaScript reference evaluators.

This module does not change Formation, Viewing, Twilight Glow, Red-Light, COT,
or any other frozen physics branch.
"""
...
PORTABLE_PACKAGE_CONTRACT_VERSION = "FIRECLOUD_ICE_OPTICS_PORTABLE_V1"
PORTABLE_PACKAGE_SCHEMA_VERSION = 1
PORTABLE_RUNTIME_DEPENDENCY = "NONE"
PORTABLE_CONSUMER = "WINDY_FIRECLOUD_OBSERVER"
PORTABLE_EVALUATOR_VERSION = "ICE_OPTICS_EVALUATOR_V1"
...
def _lookup_group(lut: pd.DataFrame, habit: str, roughness: str, reff_um: float) -> tuple[pd.DataFrame, str]:
    df = validate_ice_optics_lut(lut, require_full_six_band=True)
    sub = df[
    ...
    radii = np.array(sorted(pd.to_numeric(sub["effective_radius_um"], errors="coerce").dropna().unique()), dtype=float)
    if len(radii) == 0 or not math.isfinite(float(reff_um)):
        return pd.DataFrame(), "ICE_EFFECTIVE_RADIUS_MISSING"
    r = float(reff_um)
    if r < radii.min() - 1e-12 or r > radii.max() + 1e-12:
        return pd.DataFrame(), "ICE_REFF_OUTSIDE_LUT_DOMAIN"
    exact = np.where(np.isclose(radii, r, rtol=0.0, atol=1e-9))[0]
    if len(exact):
        rr = float(radii[int(exact[0])])
        out = sub[np.isclose(pd.to_numeric(sub["effective_radius_um"], errors="coerce"), rr, atol=1e-9, rtol=0.0)].copy()
        return out, "EXACT_REFF_LUT_ROW"
    hi = int(np.searchsorted(radii, r, side="right")); lo = hi - 1
    if lo < 0 or hi >= len(radii):
        return pd.DataFrame(), "ICE_REFF_OUTSIDE_LUT_DOMAIN"
    r0, r1 = float(radii[lo]), float(radii[hi])
    a = sub[np.isclose(pd.to_numeric(sub["effective_radius_um"], errors="coerce"), r0, atol=1e-9, rtol=0.0)].set_index("wavelength_nm")
    b = sub[np.isclose(pd.to_numeric(sub["effective_radius_um"], errors="coerce"), r1, atol=1e-9, rtol=0.0)].set_index("wavelength_nm")
    ...
    for wl in ICE_OPTICS_WAVELENGTHS_NM:
        x = a.loc[wl]; y = b.loc[wl]
        rows.append({
            "wavelength_nm": int(wl),
            "effective_radius_um": r,
            "ice_habit": str(habit),
            "surface_roughness": str(roughness),
            "mass_extinction_coefficient_m2_kg": float(x["mass_extinction_coefficient_m2_kg"] + w*(y["mass_extinction_coefficient_m2_kg"]-x["mass_extinction_coefficient_m2_kg"])),
            "single_scattering_albedo": float(x["single_scattering_albedo"] + w*(y["single_scattering_albedo"]-x["single_scattering_albedo"])),
            "asymmetry_parameter": float(x["asymmetry_parameter"] + w*(y["asymmetry_parameter"]-x["asymmetry_parameter"])),
        })
    return pd.DataFrame(rows), "LINEAR_REFF_INTERPOLATION_WITHIN_LUT"
...
def evaluate_portable_ice_optics(
    ...
    *,
    ...
    effective_radius_um: float | None,
    ice_habit: str | None,
    surface_roughness: str | None,
) -> dict[str, Any]:
    """Reference evaluator used to generate portable cross-language vectors.

    This function mirrors the standalone JS/TS consumer semantics.  It is a
    software-reference path, not a new PhysicsCore physics branch.
    """
    base: dict[str, Any] = {
        ...
        "effective_radius_um": None if effective_radius_um is None else float(effective_radius_um),
    ...
    if iwp_kg_m2 is None or not math.isfinite(float(iwp_kg_m2)):
        ...
        return base
    if native_vertical_completeness is None or not math.isfinite(float(native_vertical_completeness)) or float(native_vertical_completeness) < 1.0 - 1e-12:
        base.update(state="ICE_IWP_INCOMPLETE_NATIVE_VERTICAL_SUPPORT", missing_reason="NATIVE_VERTICAL_COMPLETENESS_LT_1")
        return base
    iwp = float(iwp_kg_m2)
    if iwp < 0.0:
    ...
    if abs(iwp) <= 1e-15:
        ...
        for wl in ICE_OPTICS_WAVELENGTHS_NM:
            base["bands"][str(wl)] = {"k_ext_m2_kg": None, "tau": 0.0, "transmission": 1.0, "ssa": None, "g": None}
        return base
    if effective_radius_um is None or not math.isfinite(float(effective_radius_um)):
        base.update(state="ICE_EFFECTIVE_RADIUS_MISSING", missing_reason="NO_NATIVE_OR_CALIBRATED_ICE_REFF")
        return base
    ...
    if not surface_roughness or str(surface_roughness).strip().upper() == "UNKNOWN":
        ...
        return base
    rows, state = _lookup_group(lut, str(ice_habit), str(surface_roughness), float(effective_radius_um))
    if rows.empty:
...
def build_reference_validation_vectors(lut: pd.DataFrame) -> dict[str, Any]:
    """Create deterministic software-validation vectors from a calibrated LUT.

    IWP values are explicitly software test inputs, not climatology or a cloud
    prior. They exist only to verify cross-language arithmetic and interpolation.
    """
    df = validate_ice_optics_lut(lut, require_full_six_band=True)
    ...
    for (habit, roughness), g in groups[:3]:
        radii = sorted(pd.to_numeric(g["effective_radius_um"], errors="coerce").dropna().unique().tolist())
        if not radii:
            continue
        candidates = [float(radii[0]), float(radii[-1])]
        if len(radii) >= 2:
            candidates.insert(1, (float(radii[0]) + float(radii[-1])) / 2.0)
        for reff in candidates:
            case_id += 1
            inp = {
                "iwp_kg_m2": 0.01,
                "native_vertical_completeness": 1.0,
                "effective_radius_um": float(reff),
                "ice_habit": str(habit),
                "surface_roughness": str(roughness),
            }
            expected = evaluate_portable_ice_optics(df, **inp)
            vectors.append({"case_id": f"READY_{case_id:03d}", "purpose": "SOFTWARE_VALIDATION_ONLY", "input": inp, "expected": expected})
    zero = {
        "iwp_kg_m2": 0.0,
        "native_vertical_completeness": 1.0,
        "effective_radius_um": None,
        "ice_habit": None,
        "surface_roughness": None,
    }
    vectors.append({"case_id": "EXACT_ZERO_IWP", "purpose": "SOFTWARE_VALIDATION_ONLY", "input": zero, "expected": evaluate_portable_ice_optics(df, **zero)})
    incomplete = {
        "iwp_kg_m2": 0.01,
    ...
    vectors.append({"case_id": "INCOMPLETE_VERTICAL_SUPPORT", "purpose": "SOFTWARE_VALIDATION_ONLY", "input": incomplete, "expected": evaluate_portable_ice_optics(df, **incomplete)})
    return {
        "validation_contract": "FIRECLOUD_ICE_OPTICS_CROSS_LANGUAGE_V1",
        "evaluator_version": PORTABLE_EVALUATOR_VERSION,
...
def portable_contract_payload(*, physicscore_version: str, science_baseline: str) -> dict[str, Any]:
    return {
        ...
        "interpolation": {
            ...
            "effective_radius": "LINEAR_WITHIN_SAME_HABIT_AND_ROUGHNESS_ONLY",
        ...
        "missing_semantics": "Missing != Clear != Zero; positive IWP requires complete vertical support, calibrated effective size, habit, roughness and LUT domain coverage.",
        "deployment_boundary": "PhysicsCore builds/calibrates/certifies the package; WINDY imports the released artifact and evaluates locally without calling PhysicsCore.",
        "physics_promotion_allowed": False,
    }


REFERENCE_EVALUATOR_MJS = r'''// Firecloud Ice Optics portable reference evaluator.
...
REFERENCE_EVALUATOR_TS = r'''// Firecloud Ice Optics portable TypeScript evaluator for WINDY/Svelte.
...
def build_portable_package_files(
    ...
    *,
    ...
    contract = portable_contract_payload(physicscore_version=physicscore_version, science_baseline=science_baseline)
    ...
    status_obj = asdict(lut_status) if lut_status is not None else {
        "loaded": True, "path": None, "row_count": len(df), "spectral_complete": True,
        "state": "ICE_OPTICS_LUT_READY", "detail": "",
    }
    manifest = {
        "portable_package_contract_version": PORTABLE_PACKAGE_CONTRACT_VERSION,
        "schema_version": PORTABLE_PACKAGE_SCHEMA_VERSION,
