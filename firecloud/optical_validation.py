    ...
    *,
    ...
    for angle in sorted(angles, reverse=True):
        ...
        rows.append({
            ...
            "scene_resolved_horizontal_support_count": int(len(resolved_support)),
            "ray_intersected_resolved_support_count": int(resolved_slant.get("cloud_layer_id", pd.Series(dtype=str)).astype(str).nunique()) if not resolved_slant.empty else 0,
