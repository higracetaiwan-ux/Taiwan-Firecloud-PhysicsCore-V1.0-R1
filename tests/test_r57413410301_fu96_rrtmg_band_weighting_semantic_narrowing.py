...
def test_step3q1_qualifies_semantic_class_but_keeps_exact_history_fail_closed():
    ...
    assert gate["qualification_state"] == "PASS_FAIL_CLOSED_V25_OFFICIAL_SCIENTIFIC_SOURCE_SEMANTIC_EQUIVALENCE_QUALIFIED_ORIGINAL_TARBALL_BYTES_HASH_UNRECOVERED_PREAVERAGING_GENERATOR_UNRECOVERED"
...
def test_step3q1_contract_records_qualified_formulas_without_promoting_exact_weighting():
    ...
    assert payload["step_version"] == "R5.7.41.3.4.10.30.13"
    ...
    assert a == b
    assert json.loads(a.decode("utf-8"))["contract_version"] == "FIRECLOUD_ICE_FU96_RRTMG_BAND_WEIGHTING_PROVENANCE_V1_13"
