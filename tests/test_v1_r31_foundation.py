...
def test_geometry_only_target_cloud_is_not_mistaken_for_upstream_blocker():
    ...
    # The target cloud sits at the endpoint and is not part of the Sun→CloudBase
    # extinction path. With complete path geometry and no upstream cloud, the
    # cloud-path component is known zero even though target optical response is
    # still GEOMETRY_ONLY/unknown.
    assert (out['spectral_optical_paths']['tau_cloud'] == 0.0).all()
