...
def test_step3q_pins_lineage_but_keeps_exact_weighting_fail_closed():
    ...
    assert gate["qualification_state"] == "PASS_FAIL_CLOSED_V25_OFFICIAL_DOWNLOAD_ENDPOINT_AND_EXTRACTED_FOOTPRINT_QUALIFIED_ORIGINAL_TARBALL_BYTES_HASH_UNRECOVERED_PREAVERAGING_GENERATOR_UNRECOVERED"
...
def test_step3q_contract_is_stable_nonproduction_and_versioned():
    ...
    assert payload["step_version"] == "R5.7.41.3.4.10.30.12"
    ...
    assert a == b
    assert json.loads(a.decode("utf-8"))["contract_version"] == "FIRECLOUD_ICE_FU96_RRTMG_BAND_WEIGHTING_PROVENANCE_V1_12"
