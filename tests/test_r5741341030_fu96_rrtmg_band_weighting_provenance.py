...
def test_step3q_pins_lineage_but_keeps_exact_weighting_fail_closed():
    ...
    assert gate["qualification_state"] == "PASS_FAIL_CLOSED_HISTORICAL_PUBLIC_RELEASE_EXISTENCE_AND_CURRENT_ARCHIVE_BOUNDARY_QUALIFIED_EXACT_GENERATOR_UNRECOVERED"
...
def test_step3q_contract_is_stable_nonproduction_and_versioned():
    ...
    assert payload["step_version"] == "R5.7.41.3.4.10.30.7"
    ...
    assert a == b
    assert json.loads(a.decode("utf-8"))["contract_version"] == "FIRECLOUD_ICE_FU96_RRTMG_BAND_WEIGHTING_PROVENANCE_V1_7"
