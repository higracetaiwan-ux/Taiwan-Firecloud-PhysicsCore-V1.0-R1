...
def test_collection_guard_allows_physically_valid_zero_target_cohort():
    ...
    cm = build_case_manifest(_request(), result, program_version="1.0.0-R5.7.41.3.4.10.12.1")
    cohort = build_cohort_summary(_request(), result, program_version="1.0.0-R5.7.41.3.4.10.12.1")
    archive_manifest = pd.DataFrame([{"artifact":n} for n in [
        "shadow_validation_case_manifest.csv","shadow_validation_cohort_summary.csv",
        "shadow_validation_ground_truth_template.csv","shadow_validation_runtime_summary.csv",
    ]])
    collection = build_collection_integrity_audit(archive_manifest, cm, cohort, result["v1_canvas_cot_semantic_migration"])
    by_id = collection.set_index("check_id")["status"].to_dict()
    assert by_id["SHADOW_COLLECTION_NO_PRODUCTION_SWITCH"] == "PASS"
    assert by_id["SHADOW_COLLECTION_NO_PROMOTION"] == "PASS"
    assert by_id["SHADOW_COLLECTION_PRODUCTION_SOURCE_FROZEN"] == "PASS"
    merged = merge_collection_audit(pd.DataFrame([{"check_id":"X","status":"PASS"}]), collection)
    overall = merged.loc[merged.check_id.eq("CASE_ARCHIVE_INTEGRITY_OVERALL"), "status"].iloc[0]
    assert overall == "PASS"
...
def test_zero_target_cohort_still_fails_if_manifest_frozen_source_contract_is_tampered():
    ...
    collection = build_collection_integrity_audit(archive_manifest, cm, cohort, result["v1_canvas_cot_semantic_migration"])
    by_id = collection.set_index("check_id")["status"].to_dict()
    assert by_id["SHADOW_COLLECTION_PRODUCTION_SOURCE_FROZEN"] == "FAIL"
