...
def test_step3q6_public_archive_boundary_fail_closed():
    ...
    assert e.loc["RRTMG_SW_PRE_V5_PUBLIC_RELEASE_AVAILABILITY", "status"] == "PASS_QUALIFIED"
    ...
    assert g["qualification_state"] == "PASS_FAIL_CLOSED_PUBLIC_ARCHIVE_AVAILABILITY_BOUNDARY_QUALIFIED_EXACT_GENERATOR_UNRECOVERED"
    assert bool(g["RRTMG_SW_PUBLIC_RELEASE_AVAILABILITY_BOUNDARY_QUALIFIED"]) is True
    assert bool(g["RRTMG_SW_PRE_V5_PUBLIC_RELEASES_AVAILABLE"]) is False
...
def test_step3q6_contract_records_archive_availability_without_inference():
    ...
    assert p["contract_version"] == "FIRECLOUD_ICE_FU96_RRTMG_BAND_WEIGHTING_PROVENANCE_V1_6"
    assert p["step_version"] == "R5.7.41.3.4.10.30.6"
    assert p["qualified_weighting_semantic_class"]["rrtmg_sw_pre_v5_public_releases_available"] is False
