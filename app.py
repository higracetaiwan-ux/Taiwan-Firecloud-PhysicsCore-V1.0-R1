...
from firecloud.case_archive_stream import write_dataframe_csv_member
...
def _launch_analysis_worker(request: dict, job_state: dict):
    ...
    # R5.7.41.3.4.6: DWD raw model-level fields are large (~hundreds of MB per
    # event) and immutable for an exact run/lead/variable/level.  Keep the warm
    # production DWD raw cache under FIRECLOUD_STATE_DIR so detached workers and
    # later analyses in the same deployment can reuse it.  Cold Test still gets
    # an isolated raw namespace; only static DWD remap assets remain shared.
    _shared_provider_root = (_STATE_DIR / "provider_cache_shared").resolve()
    _shared_dwd_remap = (_shared_provider_root / "dwd_icon_remap").resolve()
    _shared_dwd_remap.mkdir(parents=True, exist_ok=True)
    env.setdefault("FIRECLOUD_DWD_ICON_REMAP_DIR", str(_shared_dwd_remap))
    if _cache_policy == "ISOLATED_JOB_NAMESPACE" and _cache_ns_job:
        ...
        job_state["dwd_persistent_raw_cache_namespace"] = str(_provider_root / "dwd_icon")
    else:
        _shared_dwd_raw = (_shared_provider_root / "dwd_icon_raw").resolve()
        _shared_dwd_raw.mkdir(parents=True, exist_ok=True)
        env["FIRECLOUD_DWD_ICON_CACHE_DIR"] = str(_shared_dwd_raw)
        job_state["provider_cache_namespace"] = "SHARED_PROVIDER_DEFAULTS"
        job_state["dwd_persistent_raw_cache_namespace"] = str(_shared_dwd_raw)
    previous_attempt = {
...
if run or st.session_state.analysis_result is not None:
    ...
    def _zip_write_csv_stream(zf, arcname, df, *, chunksize=8192):
        """Stream CSV with bounded buffered ZIP writes and exact payload hash."""
        return write_dataframe_csv_member(zf, arcname, df, chunksize=chunksize)
    ...
    def _build_case_archive_bytes():
        ...
        _case_pre_export_elapsed = perf_counter() - _t0
        ...
        with zipfile.ZipFile(_mem, "w", zipfile.ZIP_DEFLATED, compresslevel=1, allowZip64=True) as z:
            ...
            _case_csv_elapsed = perf_counter() - _case_csv_t0

            _offset = len(_items)
            _case_json_t0 = perf_counter()
            ...
            _case_json_elapsed = perf_counter() - _case_json_t0
            ...
            _extra_perf = pd.DataFrame([
                ...
                {"stage": "CASE_EXPORT_CSV_MEMBERS", "elapsed_seconds": _case_csv_elapsed, "cache_status": "COMPUTED_STREAMING_BUFFERED_4MIB", "detail": "Bounded 4 MiB buffered DataFrame→CSV→ZipExtFile path."},
                {"stage": "CASE_EXPORT_JSON_MEMBERS", "elapsed_seconds": _case_json_elapsed, "cache_status": "COMPUTED_JSON_DIRECT", "detail": "JSON payload serialization before manifest/integrity self-audit."},
...
st.divider()
st.caption(
    "R5.7.41.3.4.6 Runtime / I-O Hardening：DWD warm-production raw cache 改為 shared persistent namespace，exact model/product/grid/run/lead/variable/level identity + QC stamp + byte-size + SHA256 guard；修正 DWD API efficiency audit 的 OK_DOWNLOADED 漏算；CASE CSV 使用 bounded 4 MiB buffered ZIP streaming；新增 CASE export 與 aggregation 細分 profiler。全部為工程 telemetry / I-O 最佳化，不改 science。"
